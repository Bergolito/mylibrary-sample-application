...
import { EntityArrayResponseType, LivroService } from '../service/livro.service';
...
export class LivroComponent implements OnInit {
  ...
  trackId = (_index: number, item: ILivro): number => this.livroService.getLivroIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.livros = this.refineData(dataFromBody);
  }

  protected refineData(data: ILivro[]): ILivro[] {
    const { predicate, order } = this.sortState();
    return predicate && order ? data.sort(this.sortService.startSort({ predicate, order })) : data;
  }

  protected fillComponentAttributesFromResponseBody(data: ILivro[] | null): ILivro[] {
    return data ?? [];
  }
